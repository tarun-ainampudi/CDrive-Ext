...
function answerCurrentQuestion() {
    ...
    console.log(`[Default] [Debug] decryptedKeyData: ${JSON.stringify(decryptedKeyData)}`);

    const sectionsInfo = getSectionInfo();
    if (sectionsInfo.length === 0) {
        console.log("[Default] [Debug] No sections found. Cannot auto-fill MCQs.");
        return;
    }
    ...
    const secIndex = parseInt(secNameDiv.textContent.match(/Section (\d+)\/\d+/)[1]) - 1;
    const qNo = header.match(/Question No : (\d+) \/ \d+/);
    const num = qNo ? qNo[1] : null;
    if (num && !isNaN(secIndex)) {
        const opIndex = decryptedKeyData.at(-1)[parseInt(num) + sectionsInfo[secIndex].startQIndex - 1];
        const opDiv = document.querySelector("#tt-option-" + opIndex + " > label > span.checkmark1");
        if (opDiv) {
            opDiv.dispatchEvent(new Event("click", { bubbles: true }));
        }
    } else {
        console.log(`[Default] [Debug] Question Number: ${num} Section Index: ${secIndex}`);
    }
}
...
async function answerMcqDefault() {
    ...
    let queArray = [...document.querySelectorAll("div[aria-labelledby='each-question']")];
    let currSelectedQue = document.querySelector('div[aria-labelledby^="currentQuestion"]');
    ...
    console.log("[Default] Answering Questions. Press backspace to stop the script.");

    for (let i = currentIndex; i < queArray.length && isDefaultMcqSolverRunning; i++) {
    ...
    console.log("[Default] All questions answered. Passing time press backspace to stop the script.");

    for (let i = 0; i < queArray.length && isDefaultMcqSolverRunning; i++) {
        const passTime = Math.floor(Math.random() * 50000) + 30000;
        console.log(`[Default] Passing ${Math.floor(passTime / 1000)} Seconds On Question: ${i + 1}`);
        queArray[i].querySelector("div").click()
...
window.addEventListener('message', (event) => {
    ...
    if (data.action !== undefined && data.action === 'key_data') {
        console.log(
            `[Default] [Debug] data: ` + `${JSON.stringify(data)}`
        );
