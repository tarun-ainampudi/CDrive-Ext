...
function checkAllSolutions() {
    const solCheckBoxes = document.querySelectorAll(
        'div[aria-labelledby="show-sol-check-cont"]'
    );
...
async function resolveCodesResponse() {
    return new Promise((resolve, reject) => {
        const timeout = setTimeout(() => {
            ...
            reject(
                new Error('Timeout reached waiting for codes from ace_helper')
            );
        }, 2000);

        const handleMessage = (event) => {
            const data = event.data;
            if (data.action !== undefined && data.action === 'ace_codes') {
                console.log(
                    `[COD Result] [Debug] data: ` + `${JSON.stringify(data)}`
                );
...
function mapEditorsToIndex(divs) {
    ...
    const indexArray = [...divs].map((editor) => {
...
async function injectAceHelper() {
    if (!isAceHelperInjected) {
        ...
        console.log(`[COD Result] [Debug] runtime ` + `response: ${msg}`);
        return;
    }
...
async function getSolutionsForCode(div) {
    const solutions = {};
    const answerHeaders = div.querySelectorAll(
...
async function getRCodesInCPage() {
    ...
    for (let i = 0; i < codeDivs.length; i++) {
        ...
        const solutionsDiv = div.querySelector(
            'div[aria-labelledby="correct-answer-mark-cont"]'
        );
