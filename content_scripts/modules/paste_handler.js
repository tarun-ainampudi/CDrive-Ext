...
async function injectTextInToInput(text) {
    ...
    if (inputElement !== null && inputElement.value !== undefined) {
        isTyperActive = true;
        for (let i = 0; i < text.length; i++) {
            ...
            const newText =
                input.substring(0, start) + char + input.substring(end);
...
function getInputElementInPage() {
    ...
    if (!testType.includes('program') && !testType.includes('cod'))
    ...
    const answerEditors = document.querySelectorAll(
        'div[aria-labelledby="editor-answer"].ace_editor'
    );
    if (answerEditors.length === 1) {
        console.log(
            `[Paste Handler] Input Element Selected By filteredAEds.length === 1`
        );
    ...
    if (textInputs.length === 0) {
        ...
        if (textInputs.length === 1) {
            console.log(
                `[Paste Handler] Input Element Selected By textInputs.length === 1`
            );
            return textInputs[0];
        } else {
            console.log(`[Paste Handler] Random selection of input`);
            return textInputs[
                Math.floor((textInputs.length - 1) * Math.random())
            ];
        }
    }
    return null;
