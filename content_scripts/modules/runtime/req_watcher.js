function getDecryptionKey() {
    ...
    if (acdEvent === null || sclDetails === null) {
        console.log("[Req Watcher] [Error] Required Local Storage Items Not Found");
        return "";
    }
...
function decryptPayload(encPayload, key) {
    ...
    if (parsedPayload['data'] === undefined) {
        console.log("[Req Watcher] [Error] Payload Doesn't Contain Data to Decrypt");
        return null;
    }
    ...
    function PatchedXMLHttpRequest() {
        ...
        xhr.addEventListener("readystatechange", function () {
            if (xhr.readyState === 4) {
                ...
                console.log(`[Req Watcher] [Debug] Response URL: ${responseUrl}`);
                if (
                    responseUrl.includes("https://api.examly.io/api/sEKMRyOJKjIzZbUa") ||
                    responseUrl.includes("https://api.examly.io/api/9DECJfxqhu0cgJAQ")
                ) {
                    console.log(`[Req Watcher] [Debug] URL Matched`);
                    const decKey = getDecryptionKey();
                    if (!decKey) {
                        console.log("[Req Watcher] [Error] Decryption Key Not Found");
                        return;
                    }
                    const decRes = decryptPayload(xhr.responseText, decKey);
                    if (decRes["frozen_test_data"] === undefined) {
                        console.log("[Req Watcher] [Error] frozen_test_data Not Found");
                        return;
                    }
                    console.log(`[Req Watcher] [Debug] frozen_test_data: ${JSON.stringify(decRes["frozen_test_data"])}`);
                    const kData = getKeyData(decRes["frozen_test_data"]);
                    if (kData.length === 0) {
                        console.log("[Req Watcher] [Error] keyData length === 0");
                        return;
                    }
